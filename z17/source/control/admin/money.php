...
class control extends adminbase
{
    ...
    public function control_run( )
    {
        ...
        TPL::display( $this->cptpl."money.tpl" );
    }

    public function control_add( )
    {
        $this->checkAuth( "money_add" );
    ...
    public function control_saveadd( )
    {
        ...
        if ( TRUE === $result )
        {
            ...
            {
                XHandle::halt( "操作成功", $this->cpfile."?c=money", 0 );
            }
        }
        else
        {
            $this->log( "money_add", "", 0 );
            XHandle::halt( "操作失败", "", 1 );
        }
    }
    ...
    {
        $args = XRequest::getgpc( array( "userid", "actiontype", "amount", "ordernum", "logcontent" ) );
        ...
        if ( $args['amount'] < 0 || $args['amount'] == 0 )
        {
            XHandle::halt( "操作金额必须大于0", "", 1 );
        }
        ...
        if ( $args['actiontype'] == 2 )
        {
            ...
            else if ( $user_data['money'] < $args['amount'] )
            {
                XHandle::halt( "对不起，会员现金余额不足，无法执行扣除！", "", 1 );
            }
        }
