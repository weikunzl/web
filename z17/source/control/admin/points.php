...
class control extends adminbase
{
    ...
    public function control_run( )
    {
        ...
        TPL::display( $this->cptpl."points.tpl" );
    }

    public function control_add( )
    {
        $this->checkAuth( "points_add" );
    ...
    public function control_saveadd( )
    {
        ...
        if ( TRUE === $result )
        {
            ...
            {
                XHandle::halt( "操作成功", $this->cpfile."?c=points", 0 );
            }
        }
        else
        {
            $this->log( "points_add", "", 0 );
            XHandle::halt( "操作失败", "", 1 );
        }
    }
    ...
    {
        $args = XRequest::getgpc( array( "userid", "actiontype", "points", "ordernum", "logcontent" ) );
        ...
        {
            XHandle::halt( "请选择积分去向", "", 1 );
        }
        if ( $args['points'] < 0 || $args['points'] == 0 )
        {
            XHandle::halt( "操作积分必须大于0", "", 1 );
        }
        ...
        if ( $args['actiontype'] == 2 )
        {
            ...
            else if ( $user_data['points'] < $args['points'] )
            {
                XHandle::halt( "对不起，会员剩余积分不足，无法执行扣除！", "", 1 );
            }
        }
