...
if ($this->type == 'my') {
list($datacount,$data) = $model->getMyList(array('page'=>$this->page,'pagesize'=>$this->pagesize,'searchsql'=>$searchsql));
}
elseif ($this->type == 'listen') {
list($datacount,$data) = $model->getListenList(array('page'=>$this->page,'pagesize'=>$this->pagesize,'searchsql'=>$searchsql));
}
else {
if ($this->uid >0) {
$searchsql .= "AND v.userid='".$this->uid."'";
}
list($datacount,$data) = $model->getAllList(array('page'=>$this->page,'pagesize'=>$this->pagesize,'searchsql'=>$searchsql));
}
