...
class hometownAModel extends X {
public function getList() {
$cache = parent::import('cache','lib');
$data = $cache->readCache('hometown');
unset($cache);
if (!empty($data)) {
$total = count($data);
}
...
$sql = "SELECT * FROM ".DB_PREFIX."hometown WHERE areaid='{$id}'";
return parent::$obj->fetch_first($sql);
}
public function doAdd($array) {
$areaid = parent::$obj->fetch_newid('SELECT MAX(areaid) FROM '.DB_PREFIX.'hometown',1);
...
$result = parent::$obj->insert(DB_PREFIX.'hometown',$array);
if (true === $result) {
$cache = parent::import('cache','lib');
$cache->updateCache('hometown');
unset($cache);
}
return $result;
}
...
$array = array_merge($array,array('depth'=>$depth));
$result = parent::$obj->update(DB_PREFIX.'hometown',$array,"areaid='{$id}'");
if (true === $result) {
...
if ($chilid>0) {
parent::$obj->query("DELETE FROM ".DB_PREFIX."hometown WHERE `areaid`='{$chilid}'");
}
}
}
$sql = "DELETE FROM ".DB_PREFIX."hometown WHERE areaid='{$id}'";
...
if ($type == 'flagopen') {
return parent::$obj->query("UPDATE ".DB_PREFIX."hometown SET flag='1' WHERE areaid='".intval($id)."'");
}
elseif ($type == 'flagclose') {
return parent::$obj->query("UPDATE ".DB_PREFIX."hometown SET flag='0' WHERE areaid='".intval($id)."'");
}
}
public function checkExistsChild($id) {
$sql = "SELECT COUNT(areaid) FROM ".DB_PREFIX."hometown WHERE rootid='".intval($id)."'";
...
$sql = "SELECT depth FROM ".DB_PREFIX."hometown WHERE areaid='".intval($id)."'";
...
$args = $args;
$query = "SELECT areaid,areaname FROM ".DB_PREFIX."hometown WHERE rootid='".intval($id)."'";
...
$query = "SELECT areaid FROM ".DB_PREFIX."hometown WHERE rootid='".intval($id)."'";
$data = parent::$obj->getall($query);
foreach($data as $key=>$value) {
$child_depth = $depth+1;
...
$notes = array();
$i = 0;
$sql = "SELECT * FROM ".DB_PREFIX."hometown WHERE rootid='0' ORDER BY orders ASC";
...
$nodes = array();
$nodes = $data;
$child_sql = "SELECT * FROM ".DB_PREFIX."hometown".
...
$notes = '';
$parent_sql = "SELECT areaid,areaname FROM ".DB_PREFIX."hometown".
...
$args = $args;
$child_sql = "SELECT areaid,areaname,depth FROM ".DB_PREFIX."hometown".
" WHERE rootid='$rootid' ORDER BY orders ASC";
$child_data = parent::$obj->getall($child_sql);
foreach ($child_data as $key=>$value) {
$args .= "<option value='".$value['areaid']."'";
...
$temps = '';
$sql = "SELECT areaid,areaname FROM ".DB_PREFIX."hometown WHERE rootid='0' AND flag='1' ORDER BY orders ASC";
...
$temps = '';
$sql = "SELECT areaid,areaname FROM ".DB_PREFIX."hometown WHERE `rootid`='{$rootid}' AND `flag`='1' ORDER BY orders ASC";
$data = parent::$obj->getall($sql);
foreach ($data as $key=>$value) {
$temps .= "<option value='".$value['areaid']."'";
if (intval($inval) == $value['areaid']) {
...
public function getCacheHomeTown($id) {
$cache = parent::import('cache','lib');
$data = $cache->readCache('hometown');
unset($cache);
$areaname = '';
...
public function getCacheHomeTownID($name) {
$cache = parent::import('cache','lib');
$data = $cache->readCache('hometown');
unset($cache);
$id = 0;
