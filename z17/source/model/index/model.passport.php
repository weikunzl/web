...
class passportIModel extends X {
	public function doReg($main_args, $profile_args, $contact_args, $params_args, $status = 0) {
		...
		$main_args = array_merge ( array ('userid' => $userid, 'salt' => XHandle::getRndChar ( 8 ), 'groupid' => intval ( parent::$cfg ['usergrid'] ), 'integrity' => $status ), $main_args );
		$result = parent::$obj->insert ( DB_PREFIX . 'user', $main_args );
		$flag = intval ( parent::$cfg ['useraudit'] ) == 1 ? 0 : 1;
		$status_args = array ('userid' => $userid, 'regtime' => $time, 'regip' => XRequest::getip (), 'logintime' => $time, 'logintimes' => 1, 'loginip' => XRequest::getip (), 'lovestatus' => 1, 'flag' => $flag );
		...
		$vip_args = array ('userid' => $userid, 'viplevel' => intval ( parent::$cfg ['usergrid'] ), 'vipforever' => 0 );
		parent::$obj->insert ( DB_PREFIX . 'user_validate', $vip_args );
		if (empty ( $contact_args )) {
			$contact_args = array ('userid' => $userid, 'privacy' => 1 );
		} else {
			$contact_args = array_merge ( array ('userid' => $userid, 'privacy' => 1 ), $contact_args );
		}
		parent::$obj->insert ( DB_PREFIX . 'user_attr', $contact_args );
		$profile_args = array_merge ( array ('userid' => $userid ), $profile_args );
		parent::$obj->insert ( DB_PREFIX . 'user_profile', $profile_args );
		$params_args = array_merge ( array ('userid' => $userid, 'lovestatus' => $status, 'groupid' => intval ( parent::$cfg ['usergrid'] ), 'flag' => $flag, 'addtime' => $time, 'ontime' => $time ), $params_args );
		parent::$obj->insert ( DB_PREFIX . 'user_params', $params_args );
		if (true === $result && intval ( parent::$cfg ['useraudit'] ) == 0) {
			$cookies_array = array ('userid' => $userid, 'username' => $main_args ['username'], 'password' => $main_args ['password'], 'lasttime' => $time );
			$this->_setCookies ( $cookies_array );
			if (true === XValid::isEmail ( $main_args ['email'] )) {
				$em = parent::model ( 'mail', 'am' );
				$em_result = $em->sendReg ( array ('userid' => $userid, 'username' => $main_args ['username'], 'password' => $origin_password, 'email' => $main_args ['email'] ) );
				$em->sendValid ( $userid );
				unset ( $em );
			}
		}
		...
		$cond_array = array ('gender' => $main_args ['gender'], 'ageyear' => $profile_args ['ageyear'], 'height' => $profile_args ['height'], 'province' => $profile_args ['provinceid'], 'city' => $profile_args ['cityid'] );
	...
	public function doPerfect($main_args, $profile_args, $params_args) {
		...
		if (true === $result) {
			...
			$time = time ();
			$params_args = array_merge ( array ('lovestatus' => 1, 'groupid' => 1, 'flag' => 1, 'addtime' => $time, 'ontime' => $time ), $params_args );
	...
	public function doLogin($loginname, $password) {
		...
		$sql = "SELECT v.userid, v.username, v.password, s.flag" . " FROM " . DB_PREFIX . "user AS v" . " LEFT JOIN " . DB_PREFIX . "user_status AS s ON s.userid=v.userid";
		...
		if (! empty ( $data )) {
			if (intval ( $data ['flag'] ) == 0) {
				...
				$last_time = time ();
				$array = array ('logintime' => $last_time, 'logintimes' => '[[logintimes+1]]', 'loginip' => XRequest::getip () );
				...
				$status = 3;
				$cookies_array = array ('userid' => $data ['userid'], 'username' => $data ['username'], 'password' => $data ['password'], 'lasttime' => $last_time );
				$this->_setCookies ( $cookies_array );
	...
	private function _match($uid) {
		...
		$lastdate = 0;
		$sql = "SELECT `dateline` FROM " . DB_PREFIX . "user_match" . " WHERE `userid`='{$uid}'";
	...
	public function doValid($userid, $key) {
		$status = 0;
		$sql = "SELECT v.userid, v.salt, s.emailrz" . " FROM " . DB_PREFIX . "user AS v" . " LEFT JOIN " . DB_PREFIX . "user_status AS s ON v.userid=s.userid" . " WHERE v.userid='{$userid}'";
		$rows = parent::$obj->fetch_first ( $sql );
		if (! empty ( $rows )) {
			if ($rows ['emailrz'] == 1) {
				$status = 1;
	...
	public function doSetNewPass($userid, $key, $password) {
		if (true === $this->checkGetPassKey ( $userid, $key )) {
			$array = array ('password' => md5 ( $password ), 'salt' => XHandle::getRndChar ( 8 ) );
	...
	public function checkLogin() {
		...
		if (empty ( $sid )) {
        	...
        	$_SESSION['uid'] = intval($userinfo[0]);
			...
			if (empty ( $data )) {
				...
				if ($data ['username'] != $userinfo [1] || $data ['password'] != $userinfo [2]) {
					...
					parent::$wrap_user = array ('userid' => intval ( $data ['userid'] ), 'username' => $data ['username'], 'gender' => $data ['gender'], 'groupid' => $data ['groupid'], 'groupname' => $data ['groupname'], 'money' => $data ['money'], 'points' => $data ['points'], 'mbsms' => $data ['mbsms'] );
					$status = true;
				}
			}
			unset ( $data );
	...
	private function _getUser($uid) {
		$sql = "SELECT v.*, g.groupname" . " FROM " . DB_PREFIX . "user AS v" . " LEFT JOIN " . DB_PREFIX . "user_group AS g ON v.groupid=g.groupid" . " WHERE v.userid='" . intval ( $uid ) . "'";
	...
	private function _regPoints($userid) {
		...
		if ($regpoints > 0) {
			$array = array ('userid' => $userid, 'actiontype' => 1, 'points' => $regpoints, 'logcontent' => XLang::get ( 'points_reglog' ), 'opuser' => '', 'ordernum' => '' );
	...
	private function _regMoney($userid) {
		...
		if ($regmoney > 0) {
			$array = array ('userid' => $userid, 'actiontype' => 1, 'amount' => $regmoney, 'logcontent' => XLang::get ( 'money_reglog' ), 'opuser' => '', 'ordernum' => '' );
	...
	private function _updateGroupLogins($uid) {
		...
		$vip_sql = "SELECT `viplevel`, `vipenddate`" . " FROM " . DB_PREFIX . "user_validate" . " WHERE `userid`='{$uid}'";
		$vip_rows = parent::$obj->fetch_first ( $vip_sql );
		if (empty ( $vip_rows )) {
			$vip_array = array ('userid' => $uid, 'viplevel' => $gid, 'vipforever' => 0, 'vipstartdate' => 0, 'vipenddate' => 0 );
			parent::$obj->insert ( DB_PREFIX . 'user_validate', $vip_array );
		} else {
			if ($vip_rows ['viplevel'] > 1) {
				if (intval ( $vip_rows ['vipenddate'] ) < $dateline) {
					$gid = 1;
					$vip_array = array ('viplevel' => $gid, 'vipforever' => 0, 'vipstartdate' => 0, 'vipenddate' => 0 );
		...
		unset ( $vip_sql );
		$can = false;
		$login_sql = "SELECT `userid`, `logindate` FROM " . DB_PREFIX . "user_logins" . " WHERE `userid`='{$uid}'";
		$login_rows = parent::$obj->fetch_first ( $login_sql );
		if (empty ( $login_rows )) {
			$login_array = array ('userid' => $uid, 'logindate' => $dateline );
			parent::$obj->insert ( DB_PREFIX . 'user_logins', $login_array );
			$can = true;
		} else {
			if (intval ( $login_rows ['logindate'] ) == $dateline) {
			} else {
				$login_array = array ('logindate' => $dateline );
		...
		if (true === $can) {
			...
			if ($points > 0) {
				...
				$log_array = array ('userid' => $uid, 'actiontype' => 1, 'points' => $points, 'logcontent' => $log_content, 'opuser' => parent::$wrap_user ['username'] );
	...
	public function saveCond($userid, $args) {
		$cond_args = array ();
		$cond_setarea [] = array ('orders' => 1, 'province' => $args ['province'], 'city' => $args ['city'] );
	...
	public function saveListen($uid) {
		...
		if (empty ( $listen_rows )) {
			...
			$listen_array = array ('friendid' => $friendid, 'userid' => $uid, 'fuserid' => $uid, 'timeline' => time (), 'flag' => 1, 'black' => 0, 'ftype' => 1 );
	...
	public function doSaveMonolog($id, $content) {
		...
		$array = array ('monolog' => $content, 'moluptime' => time (), 'molstatus' => $flag );
	...
	public function doSaveAvatar($id, $avatar) {
		...
		$array = array ('avatar' => $avatar, 'avatarflag' => $avatarflag );
	...
	public function getLoginUserInfo($id) {
		$sql = "SELECT v.*, s.*, vip.*, e.*, p.*, l.sortname" . " FROM " . DB_PREFIX . "user AS v" . " LEFT JOIN " . DB_PREFIX . "user_status AS s ON s.userid=v.userid" . " LEFT JOIN " . DB_PREFIX . "user_validate AS vip ON vip.userid=v.userid" . " LEFT JOIN " . DB_PREFIX . "user_attr AS e ON e.userid=v.userid" . " LEFT JOIN " . DB_PREFIX . "user_profile AS p ON p.userid=v.userid" . " LEFT JOIN " . DB_PREFIX . "love_sort AS l ON l.sortid=p.lovesort" . " WHERE v.userid = '{$id}'";
		$data = parent::$obj->fetch_first ( $sql );
		if (! empty ( $data )) {
	...
	public function checkMobileCode($mobile, $key) {
		$result = false;
		$sql = "SELECT `id`, `checkcode` FROM " . DB_PREFIX . "mobile_checkcode" . " WHERE `mobile`='{$mobile}'";
		$rows = parent::$obj->fetch_first ( $sql );
		if (! empty ( $rows )) {
	...
	public function updateMobileCode($mobile) {
		$array = array ('checkcode' => XHandle::getRndChar ( 6, 1 ), 'updatetime' => time () );
