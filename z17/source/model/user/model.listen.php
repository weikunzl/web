...
$status = false;
$sql = "SELECT `friendid` FROM ".DB_PREFIX."friend".
" WHERE `userid`='{$uid}' AND `fuserid`='{$fuid}' AND `black`='0' LIMIT 1";
$rows = parent::$obj->fetch_first($sql);
if (!empty($rows)) {
$status = true;
}
...
$status = false;
$sql = "SELECT `friendid` FROM ".DB_PREFIX."friend".
" WHERE `userid`='{$uid}' AND `fuserid`='{$fuid}' AND `black`='1'";
$rows = parent::$obj->fetch_first($sql);
if (!empty($rows)) {
$status = true;
}
...
$arr = null;
$sql = "SELECT `black` FROM ".DB_PREFIX."friend".
" WHERE userid='{$uid}' AND `fuserid`='{$fuid}'";
$rows = parent::$obj->fetch_first($sql);
if (!empty($rows)) {
$arr = array(
'fuid'=>$fuid,
...
$sql = "SELECT * FROM ".DB_PREFIX."friend".
" WHERE `userid`='".parent::$wrap_user['userid']."' AND `fuserid`='{$tuid}'";
$rows = parent::$obj->fetch_first($sql);
if (!empty($rows)) {
if ($rows['black'] == 0) {
$result = 3;
...
$query = "DELETE FROM ".DB_PREFIX."friend".
...
$query = "SELECT `friendid` FROM ".DB_PREFIX."friend".
" WHERE `userid`='".parent::$wrap_user['userid']."'".
" AND `fuserid`='{$tuid}'";
...
$sql = "SELECT `black` FROM ".DB_PREFIX."friend".
" WHERE `fuserid`='{$uid}' AND `userid`='".parent::$wrap_user['userid']."'";
$rows = parent::$obj->fetch_first($sql);
if (!empty($rows)) {
if ($rows['black'] == 1) {
$flag = 2;
...
$pals_sql = "SELECT `pals_id` FROM ".DB_PREFIX."chat_pals".
" WHERE `host_id`='".parent::$wrap_user['userid']."'".
" AND `pals_id`='{$fuid}'";
$pals_rs = parent::$obj->fetch_first($pals_sql);
if (empty($pals_rs)) {
$pals_array = array(
'host_id'=>parent::$wrap_user['userid'],
'pals_id'=>$fuid,
);
...
if (true === parent::$cfg['webim_enable']) {
if (true === parent::$obj->table_exist('chat_pals')) {
$pals_sql = "DELETE FROM ".DB_PREFIX."chat_pals".
" WHERE `host_id`='".parent::$wrap_user['userid']."'".
" AND `pals_id`='{$fuid}'";
...
if (true === parent::$cfg['webim_enable']) {
if (true === parent::$obj->table_exist('chat_pals')) {
$count_sql = "SELECT COUNT(*) AS my_count FROM ".DB_PREFIX."chat_pals".
" WHERE `host_id`='".parent::$wrap_user['userid']."'";
