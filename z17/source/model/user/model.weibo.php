...
$items['orderby'] = empty($items['orderby']) ?' ORDER BY v.wbid DESC': $items['orderby'];
$pagesize   = intval($items['pagesize']);
$where      = " WHERE `flag`='1' AND v.userid IN (".
"SELECT `fuserid` FROM ".DB_PREFIX."friend".
...
$sql = "SELECT v.*, u.username, u.gender, u.avatar, u.avatarflag".
" FROM ".DB_PREFIX."weibo_comment AS v".
" LEFT JOIN ".DB_PREFIX."user AS u ON v.cmuserid=u.userid".
" WHERE v.wbid='{$wbid}' AND v.cmflag='1' ORDER BY v.cmid DESC";
$data = parent::$obj->getall($sql);
$i = 1;
...
$result = false;
$sql = "SELECT v.cmid FROM ".DB_PREFIX."weibo_comment AS v".
" LEFT JOIN ".DB_PREFIX."weibo AS w ON v.wbid=w.wbid".
" WHERE v.cmid='{$id}' AND w.userid='".parent::$wrap_user['userid']."'";
$rows = parent::$obj->fetch_first($sql);
if (!empty($rows)) {
