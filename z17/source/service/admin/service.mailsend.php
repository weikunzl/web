...
class mailsendAService extends X
{
    ...
    public function validGroup( )
    {
        ...
        {
            $m = parent::model( "mailtpl", "am" );
            $tpl_data = $m->getData( $tplid );
            unset( $m );
            if ( empty( $tpl_data ) )
            {
                XHandle::halt( "对不起，你选择的邮件模板不存在！", "", 1 );
            }
            else
            {
                $args['subject'] = $tpl_data['subject'];
                $args['content'] = $tpl_data['content'];
            }
        }
        $sqltemp = "";
        $sqltemp = " WHERE 1=1";
    ...
    public function validSingle( )
    {
        $args = array( );
        $recemail = XRequest::getargs( "recemail" );
        $tplid = XRequest::getint( "tplid" );
        $subject = XRequest::getargs( "subject" );
        $content = XRequest::getargs( "content", "", FALSE );
        if ( empty( $recemail ) )
        {
            XHandle::halt( "接收邮箱不能为空，多个请用,号隔开。", "", 1 );
        }
        if ( $tplid == 0 )
        {
            if ( empty( $subject ) )
